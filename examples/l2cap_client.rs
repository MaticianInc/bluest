...
use bluest::{Adapter, Device, Uuid as BluestUUID};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    read_stream(device).await;
    Ok(())
}

#[cfg(not(feature = "tokio"))]
async fn read_stream(device: Device) {
    use futures_util::AsyncReadExt;

    let mut channel = device.open_l2cap_channel(PSM, false).await.unwrap();
...
async fn read_stream(device: Device) {
    ...
    let mut channel = device.open_tokio_l2cap_channel(PSM, false).await.unwrap();
